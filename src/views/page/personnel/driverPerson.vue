  ...
  <div class="app-container">
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中" fit highlight-current-row @selection-change="handleSelectionChange"
      ...
      <el-table-column align="center" label='序号' min-width="75" type="index"></el-table-column>
        ...
        <el-table-column label="认证状态" min-width="115" align="center" prop="authStauts"></el-table-column>
    ...
    <el-dialog class="watchDialog" title="司机管理" :visible.sync="dialogTableVisible">
        ...
        <!-- <el-form label-width="80px">
            <el-form-item label="状态">
              <el-radio-group v-model="status" @change="examine=true">
                <el-radio :label="false">禁用</el-radio>
                <el-radio :label="true">解禁</el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item>
...
export default {
  ...
  methods: {
    async getData () {
      ...
      const { code, data, err } = await api.driverPagination({ currentPage: this.currentPage, keyWord: this.form.keyWord, pageSize: this.pageSize, recyclerPointId: this.form.recyclerPointId })
    ...
    async watchFor (id) {
      ...
      this.examine = false
      this.status = ''
      const { code, data, err } = await api.driverDetail({ id: this.watchId })
      if (code === 200) {
    ...
    submitForm (formName) {
      if (this.examine) {
        api.driverChangeStatus({ status: this.status, id: this.watchId }).then((res) => {
          if (res.code === 200) {
            this.getData()
            this.dialogTableVisible = false
            this.$message.success('成功修改司机状态')
          } else {
