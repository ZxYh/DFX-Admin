<template>
  <div class="app-container">
          ...
          <el-row>
            ...
            <el-col :span="6">
              <el-form-item label="选择司机">
                <el-select v-model="form.driverId" placeholder="选择">
                  <el-option v-for="(i, index) in form.driverList" :key="index" :label="i.name" :value="i.id"></el-option>
                  <el-option label="不限" value=""></el-option>
                </el-select>
              </el-form-item>
            </el-col>
    ...
    <el-dialog class="watchDialog" title="司机订单管理" :visible.sync="dialogTableVisible">
      ...
      async getDriver () {
        ...
        if (code === 200) {
          api.driverPagination({ currentPage: 1, pageSize: data.total }).then(res => {
            if(res.code === 200) {
              this.form.driverList = res.data.array
            }
          })
        }
      },
