<template>
  <div class="app-container">
    ...
    <el-dialog class="watchDialog" title="自有回收者订单管理" :visible.sync="dialogTableVisible">
      ...
      <div class="dialog-container" v-show="watchContent.status==='已完成'">
        <div class="flex-between">
          <span class="dialog-title">客户评价</span>
          <span class="dialog-title evaluate-title">{{watchContent.evaTitle}}</span>
        </div>
        <div class="dialog-container dialog-eva">
          <div class="dialog-remarks">{{watchContent.evaContent}}</div>
        </div>
      </div>
...
<script>
  ...
  import apiOwn from '@/api/personnel'
  ...
  export default {
    ...
    methods: {
      async getData() {
        ...
        const { code, data, err } = await api.orderPagination({ currentPage: this.currentPage, pageSize: this.pageSize, address: this.form.address, ifOwn: true, merhantId: this.form.merhantId, orderCode: this.form.orderCode, phone: this.form.phone, scrapTypeId: this.form.scrapTypeId, status: this.form.status, recyclerPointId: this.form.pointId })
      ...
      async getOwn () {
        const { code, data, err } = await apiOwn.recyclerPagination({ currentPage: 1, pageSize: 10, ifOwn: true })
        if (code === 200) {
          apiOwn.recyclerPagination({ currentPage: 1, pageSize: data.total, ifOwn: true }).then(res => {
            if(res.code === 200) {
              this.form.recycler = res.data.array
            }
          })
        }
      },
